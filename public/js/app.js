...
function handleLogin() {
    ...
    if (AUTHORIZED_USERS[user] && AUTHORIZED_USERS[user].pin === pin) {
        ...
        // --- UPDATED ADMIN VISIBILITY LOGIC ---
        document.querySelectorAll('.admin-only').forEach(el => {
            if (isAdmin) {
                // Keep tab content hidden until clicked, show the tab buttons
...
function switchTab(tabName) {
    ...
    // STRICT SECURITY CHECK: Block non-admins from opening the expense tab
...
/* REALTIME AS-YOU-TYPE VALIDATIONS */
...
function changeLanguage() {
  ...
  // Receipt Form Labels & Placeholders
  ...
  // Payment Mode Dropdown Localization
  ...
  // Re-trigger validations and amount in words conversion with new language
...
async function handleAmountInput() {
    ...
    try {
        // ADD { cache: 'no-store' } to prevent browser from caching the old language
...
async function generateAndSave() {
  ...
  // Realtime checks prior to submit
